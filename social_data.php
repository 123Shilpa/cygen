        ...
        if ($rowcount > 0) {
            while ($row = mysql_fetch_assoc($result)) {
                if ($row["Status"]) {
                    ...
                    $_SESSION['RoleId'] = '3';
                    $_SESSION['Role'] = 'Customer';
                    //$uquery = "UPDATE login_cygen SET ProfileImage='$ProfileImage', Email='$Email',FacbookId = '$SocialMediaId', UpdatedDate='$UpdatedDate' WHERE SocialMediaId = '$SocialMediaId' AND SocialMediaType = '$SocialMediaType'";
                     $uquery = "";
                        $UpdatedDate = date('Y-m-d H:i:s');
                        if ($SocialMediaType == "Facebook") {
                            $uquery = "UPDATE login_cygen SET FullName='$FullName' , FacbookId = '$SocialMediaId' , UpdatedDate = '$UpdatedDate' , ProfileImage='$ProfileImage', IsFacbook='1' WHERE Email='$Email'";							
