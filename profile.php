...
if (null == $Id) {
 ...
 if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // // keep track validation errors
		...
		$sql = "INSERT INTO profilereport  (PatientId , PersonalDetails ) VALUES (?, ? )";
        $q = $pdo->prepare($sql);
        $q->execute(array($_SESSION['PatientId'], 7));
  ...
  $.validator.addMethod("custome_name", function(value, element){ 
    ...
    $("#profile").validate({
      ...
      messages:{
		...
		ProfileImage:{
			accept:"<strong>Please select only jpg,png,jpeg </strong>"
		},
		Country:
